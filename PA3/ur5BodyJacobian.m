...
function [Jbst]= ur5BodyJacobian(x)
    syms l1 l2 l3 l4 l5
    gst_0 = [1 0 0 l3+l5; 0 1 0 0; 0 0 1 l1+l2; 0 0 0 1];
    ...
    xi1p=inv(adjoint(gst))*xi1;
    xi2p=inv(adjoint(gst))*xi2;
    xi3p=inv(adjoint(gst))*xi3;
    xi4p=inv(adjoint(gst))*xi4;
    xi5p=inv(adjoint(gst))*xi5;
    xi6p=inv(adjoint(gst))*xi6;

    Jbst=[xi1p xi2p xi3p xi4p xi5p xi6p]
end